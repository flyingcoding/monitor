 ...
 *
 * <p><b>PR3 hotfix</b>：原先本类独占的 {@code @DirtiesContext(BEFORE_CLASS)} 和
 * {@code verifyContainerConnectivity} 已下沉到 {@link IntegrationTestBase}，所有 IT 类共享
 * fail-fast 隔离 + MySQL JDBC ping。本类不再需要重复声明，业务断言保留。
 */
class SmokeIT extends IntegrationTestBase {
    ...
    void smokeAll() {
        // 1) 4 容器 isRunning（IntegrationTestBase 的 @BeforeAll verifyContainerConnectivity
        //    已对 MySQL 做了 JDBC 直连 ping，此处仅断言其他 3 个容器的运行态）
