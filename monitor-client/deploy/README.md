...
- Network failures back off without sleeping on the collector: 2–60 seconds for ordinary errors, 300 seconds for 401/403, checked by the one-second transport pump. Repeated transport warnings are limited to once per minute.
...
The transport queue is polled every second; empty rounds make no requests. Collection retains its configured interval. Four snapshot kinds must drain promptly because the original server caches snapshots for only 30 seconds.
